...
renderer = Renderer(x, y, ViewCamera.THALES)</code></pre>
...
for x in range(0, 10):
    for y in range(0, 10):
        ...
        renderer.new_mesh(meshes[1])</code></pre>

  <h2 id="movement-and-rotation" class="text-2xl font-semibold text-gray-100 mb-2">
  ...
  <div class="relative mb-4">
    <video controls class="w-full h-auto rounded-lg shadow-md" alt="example">
      <track kind="captions" />
      <source
        src="https://res.craft.do/user/full/9158afb0-cf4f-70e7-4f73-e7b4bccede14/doc/4439b81c-461c-47c4-9ac7-34152aefea9e/2d8a9992-19dc-40f6-8d9b-789f89929a95"
        type="video/mp4"
      />
  ...
  <h2 id="how-it-works-thales" class="text-2xl font-semibold text-gray-100 mb-2">
    Renderer
  </h2>

  <h3 id="thales" class="text-xl font-semibold text-gray-100 mb-2">
    Thales
  </h3>
  <p class="text-lg text-gray-300 mb-4">
...
<p class="text-lg text-gray-300 mb-4">
    For the first question, I assumed the dot represents the 1D camera. As for the second, the answer lies in applying Thales' theorem.
</p>
<p class="text-lg text-gray-300 mb-4">
    By repeating this process with a 3D point, you can create a 3D renderer using just two applications of Thales' theorem.
</p>
...
<p class="text-lg text-gray-300 mb-4">
    In other words, when you look through the glass, you perceive a three-dimensional world, but if you were to draw what you see directly on the glass, your drawing would be in two dimensions. The glass serves as a flat surface where you project a view of a three-dimensional world, simplifying its complexity into a 2D representation.
</p>
...
<code class="language-python">def to_2d(camera, width: float, height: float):
    if camera.view == ViewCamera.THALES:
        ...
        return pygame.math.Vector2(new_x * 500 + width / 2, new_y * 500 + height / 2)</code></pre>
  ...
  <p class="text-lg text-gray-300 mb-4">
    Later, after learning more about matrices, I was able to improve my code and add camera rotation to my renderer. Let me explain how it works:
  </p>
...
<code class="language-python">def rotation_matrix_x(theta):
  ...
  <ul class="list-disc pl-6 text-gray-300 mb-4">
    <li><strong class="font-bold">c</strong> and <strong class="font-bold">s</strong> represent the cosine and sine of the angle <strong class="font-bold">theta</strong>, respectively.</li>
    <li>The returned matrix performs a rotation around the X axis.</li>
  </ul>

  <h3 class="text-xl font-semibold text-gray-100 mb-2">Calculating the Final Rotation Matrix</h3>

  <p class="text-lg text-gray-300 mb-4">
    We calculate the final rotation matrix by combining rotations around the X and Y axes. The rotation matrices are multiplied in the order Y -> X to obtain the final matrix.
  </p>
...
<code class="language-python">rotation_matrix = rotation_matrix_y(rotation_y) @ rotation_matrix_x(rotation_x)</code></pre>
  ...
  <ul class="list-disc pl-6 text-gray-300 mb-4">
    <li>The rotation matrices are multiplied in this order to apply the rotations in the correct sequence.</li>
  </ul>
...
camera_matrix[:3, 3] = camera_position</code></pre>
  ...
  <h3 class="text-xl font-semibold text-gray-100 mb-2">Calculating the View Matrix</h3>

  <p class="text-lg text-gray-300 mb-4">
    The view matrix is created by combining the rotation matrix and the camera matrix. It represents the transformation needed to convert world coordinates to camera coordinates.
  </p>
...
view_matrix = view_matrix @ camera_matrix</code></pre>
  ...
  <ul class="list-disc pl-6 text-gray-300 mb-4">
    <li>We copy the rotation matrix and multiply it by the camera matrix to get the view matrix.</li>
  </ul>

  <h3 class="text-xl font-semibold text-gray-100 mb-2">Transforming the 3D Point</h3>

  <p class="text-lg text-gray-300 mb-4">
    The 3D point is transformed by applying the view matrix. This adjusts the point based on the defined rotations and translations, giving the new position of the point from the camera's perspective.
  </p>
...
transformed_point = view_matrix @ point_3d</code></pre>
  ...
  <ul class="list-disc pl-6 text-gray-300 mb-4">
    <li>We add a homogeneous component (1) to the 3D point to use 4x4 matrix multiplication.</li>
  ...
  <p class="text-lg text-gray-300 mb-4">
    One significant limitation of this simple renderer is the absence of a <strong class="font-bold">Z-buffer</strong>. The Z-buffer, or depth buffer, is a crucial component in 3D rendering that helps manage depth information for each pixel on the screen. It ensures that objects closer to the camera obscure objects further away, thus providing proper occlusion. Without it, our renderer might display incorrect visual results where objects overlap or where the depth relationship between objects is not accurately represented.
  </p>

  <p class="text-lg text-gray-300 mb-4">
    In the future, I will address this limitation by studying data about 3D rendering. For example, with this great <a href="https://youtu.be/qSrGbVxCYeo?si=2ufH6zUgWBgM7G5V" class="text-blue-400 hover:underline">documentation video</a>.
  </p>
</main>
